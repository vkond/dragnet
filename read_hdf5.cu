...
int open_hdf5(char *filename, header* h, dedisp_float*& input, int verbose=1) {
  ...
  input=(dedisp_float *) malloc(sizeof(dedisp_float) * h->nsamp * h->nchan);
  // Reading the data
  vector<size_t> pos (2);
  pos[0] = 0;
  pos[1] = 0;
  ...
  float *outbuf = new float[h->nsamp * h->nchan];
  stokes->get2D (pos, outbuf, h->nsamp, h->nchan);
  for (long ii = 0; ii < h->nsamp; ii++) {
